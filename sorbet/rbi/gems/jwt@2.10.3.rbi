# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `jwt` gem.
# Please instead update this file by running `bin/tapioca gem jwt`.


# JSON Web Token implementation
#
# Should be up to date with the latest spec:
# https://tools.ietf.org/html/rfc7519
# JSON Web Token implementation
#
# Should be up to date with the latest spec:
# https://tools.ietf.org/html/rfc7519
#
# pkg:gem/jwt#lib/jwt/version.rb:7
module JWT
  extend ::JWT::Configuration

  private

  # Decodes a JWT to extract the payload and header
  #
  # @param jwt [String] the JWT to decode.
  # @param key [String] the key used to verify the JWT.
  # @param verify [Boolean] whether to verify the JWT signature.
  # @param options [Hash] additional options for decoding.
  # @return [Array<Hash>] the decoded payload and headers.
  #
  # pkg:gem/jwt#lib/jwt.rb:49
  def decode(jwt, key = T.unsafe(nil), verify = T.unsafe(nil), options = T.unsafe(nil), &keyfinder); end

  # Encodes a payload into a JWT.
  #
  # @param payload [Hash] the payload to encode.
  # @param key [String] the key used to sign the JWT.
  # @param algorithm [String] the algorithm used to sign the JWT.
  # @param header_fields [Hash] additional headers to include in the JWT.
  # @return [String] the encoded JWT.
  #
  # pkg:gem/jwt#lib/jwt.rb:35
  def encode(payload, key, algorithm = T.unsafe(nil), header_fields = T.unsafe(nil)); end

  class << self
    # Decodes a JWT to extract the payload and header
    #
    # @param jwt [String] the JWT to decode.
    # @param key [String] the key used to verify the JWT.
    # @param verify [Boolean] whether to verify the JWT signature.
    # @param options [Hash] additional options for decoding.
    # @return [Array<Hash>] the decoded payload and headers.
    #
    # pkg:gem/jwt#lib/jwt.rb:49
    def decode(jwt, key = T.unsafe(nil), verify = T.unsafe(nil), options = T.unsafe(nil), &keyfinder); end

    # Encodes a payload into a JWT.
    #
    # @param payload [Hash] the payload to encode.
    # @param key [String] the key used to sign the JWT.
    # @param algorithm [String] the algorithm used to sign the JWT.
    # @param header_fields [Hash] additional headers to include in the JWT.
    # @return [String] the encoded JWT.
    #
    # pkg:gem/jwt#lib/jwt.rb:35
    def encode(payload, key, algorithm = T.unsafe(nil), header_fields = T.unsafe(nil)); end

    # Returns the gem version of the JWT library.
    #
    # @return [Gem::Version] the gem version.
    #
    # pkg:gem/jwt#lib/jwt/version.rb:11
    def gem_version; end

    # Checks if the OpenSSL version is 3 or greater.
    #
    # @return [Boolean] true if OpenSSL version is 3 or greater, false otherwise.
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/version.rb:29
    def openssl_3?; end

    # Checks if there is an OpenSSL 3 HMAC empty key regression.
    #
    # @return [Boolean] true if there is an OpenSSL 3 HMAC empty key regression, false otherwise.
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/version.rb:55
    def openssl_3_hmac_empty_key_regression?; end

    # Returns the OpenSSL version.
    #
    # @return [Gem::Version] the OpenSSL version.
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/version.rb:63
    def openssl_version; end

    # Checks if the RbNaCl library is defined.
    #
    # @return [Boolean] true if RbNaCl is defined, false otherwise.
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/version.rb:39
    def rbnacl?; end

    # Checks if the RbNaCl library version is 6.0.0 or greater.
    #
    # @return [Boolean] true if RbNaCl version is 6.0.0 or greater, false otherwise.
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/version.rb:47
    def rbnacl_6_or_greater?; end
  end
end

# Base64 encoding and decoding
# @api private
#
# pkg:gem/jwt#lib/jwt/base64.rb:8
class JWT::Base64
  class << self
    # pkg:gem/jwt#lib/jwt/base64.rb:30
    def loose_urlsafe_decode64(str); end

    # Decode a string with URL-safe Base64 complying with RFC 4648.
    # Deprecated support for RFC 2045 remains for now. ("All line breaks or other characters not found in Table 1 must be ignored by decoding software")
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/base64.rb:19
    def url_decode(str); end

    # Encode a string with URL-safe Base64 complying with RFC 4648 (not padded).
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/base64.rb:12
    def url_encode(str); end
  end
end

# The Base64DecodeError class is raised when there is an error decoding a Base64-encoded string.
#
# pkg:gem/jwt#lib/jwt/error.rb:53
class JWT::Base64DecodeError < ::JWT::DecodeError; end

# JWT Claim verifications
# https://datatracker.ietf.org/doc/html/rfc7519#section-4
#
# Verification is supported for the following claims:
# exp
# nbf
# iss
# iat
# jti
# aud
# sub
# required
# numeric
#
# pkg:gem/jwt#lib/jwt/claims/audience.rb:4
module JWT::Claims
  class << self
    # Returns the errors in the claims of the JWT token.
    #
    # @param options [Array] the options for verifying the claims.
    # @return [Array<JWT::Claims::Error>] the errors in the claims of the JWT
    #
    # pkg:gem/jwt#lib/jwt/claims.rb:69
    def payload_errors(payload, *options); end

    # Checks if the claims in the JWT payload are valid.
    #
    # @param payload [Hash] the JWT payload.
    # @param options [Array] the options for verifying the claims.
    # @return [Boolean] true if the claims are valid, false otherwise
    #
    # pkg:gem/jwt#lib/jwt/claims.rb:61
    def valid_payload?(payload, *options); end

    # @deprecated Use {verify_payload!} instead. Will be removed in the next major version of ruby-jwt.
    #
    # pkg:gem/jwt#lib/jwt/claims.rb:37
    def verify!(payload, options); end

    # Checks if the claims in the JWT payload are valid.
    # @example
    #
    #   ::JWT::Claims.verify_payload!({"exp" => Time.now.to_i + 10}, :exp)
    #   ::JWT::Claims.verify_payload!({"exp" => Time.now.to_i - 10}, exp: { leeway: 11})
    #
    # @param payload [Hash] the JWT payload.
    # @param options [Array] the options for verifying the claims.
    # @return [void]
    # @raise [JWT::DecodeError] if any claim is invalid.
    #
    # pkg:gem/jwt#lib/jwt/claims.rb:52
    def verify_payload!(payload, *options); end
  end
end

# The Audience class is responsible for validating the audience claim ('aud') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/audience.rb:6
class JWT::Claims::Audience
  # Initializes a new Audience instance.
  #
  # @param expected_audience [String, Array<String>] the expected audience(s) for the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/claims/audience.rb:10
  def initialize(expected_audience:); end

  # Verifies the audience claim ('aud') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidAudError] if the audience claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/audience.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/audience.rb:27
  def expected_audience; end
end

# Responsible of validation the crit header
#
# pkg:gem/jwt#lib/jwt/claims/crit.rb:6
class JWT::Claims::Crit
  # Initializes a new Crit instance.
  #
  # @param expected_crits [String] the expected crit header values for the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/claims/crit.rb:10
  def initialize(expected_crits:); end

  # Verifies the critical claim ('crit') in the JWT token header.
  #
  # @param context [Object] the context containing the JWT payload and header.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidCritError] if the crit claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/crit.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/crit.rb:32
  def expected_crits; end
end

# Verifiers to support the ::JWT.decode method
#
# @api private
#
# pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:13
module JWT::Claims::DecodeVerifier
  class << self
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:29
    def verify!(payload, options); end
  end
end

# pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:14
JWT::Claims::DecodeVerifier::VERIFIERS = T.let(T.unsafe(nil), Hash)

# Represents a claim verification error
#
# pkg:gem/jwt#lib/jwt/claims.rb:33
class JWT::Claims::Error < ::Struct
  # pkg:gem/jwt#lib/jwt/claims.rb:33
  def message; end

  # pkg:gem/jwt#lib/jwt/claims.rb:33
  def message=(_); end

  class << self
    # pkg:gem/jwt#lib/jwt/claims.rb:33
    def [](*_arg0); end

    # pkg:gem/jwt#lib/jwt/claims.rb:33
    def inspect; end

    # pkg:gem/jwt#lib/jwt/claims.rb:33
    def keyword_init?; end

    # pkg:gem/jwt#lib/jwt/claims.rb:33
    def members; end

    # pkg:gem/jwt#lib/jwt/claims.rb:33
    def new(*_arg0); end
  end
end

# The Expiration class is responsible for validating the expiration claim ('exp') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/expiration.rb:6
class JWT::Claims::Expiration
  # Initializes a new Expiration instance.
  #
  # @param leeway [Integer] the amount of leeway (in seconds) to allow when validating the expiration time. Default: 0.
  #
  # pkg:gem/jwt#lib/jwt/claims/expiration.rb:10
  def initialize(leeway:); end

  # Verifies the expiration claim ('exp') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::ExpiredSignature] if the token has expired.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/expiration.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/expiration.rb:29
  def leeway; end
end

# The IssuedAt class is responsible for validating the issued at claim ('iat') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/issued_at.rb:6
class JWT::Claims::IssuedAt
  # Verifies the issued at claim ('iat') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidIatError] if the issued at claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/issued_at.rb:13
  def verify!(context:, **_args); end
end

# The Issuer class is responsible for validating the issuer claim ('iss') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/issuer.rb:6
class JWT::Claims::Issuer
  # Initializes a new Issuer instance.
  #
  # @param issuers [String, Symbol, Array<String, Symbol>] the expected issuer(s) for the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/claims/issuer.rb:10
  def initialize(issuers:); end

  # Verifies the issuer claim ('iss') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidIssuerError] if the issuer claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/issuer.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/issuer.rb:31
  def issuers; end
end

# The JwtId class is responsible for validating the JWT ID claim ('jti') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/jwt_id.rb:6
class JWT::Claims::JwtId
  # Initializes a new JwtId instance.
  #
  # @param validator [#call] an object responding to `call` to validate the JWT ID.
  #
  # pkg:gem/jwt#lib/jwt/claims/jwt_id.rb:10
  def initialize(validator:); end

  # Verifies the JWT ID claim ('jti') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidJtiError] if the JWT ID claim is invalid or missing.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/jwt_id.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/jwt_id.rb:32
  def validator; end
end

# The NotBefore class is responsible for validating the 'nbf' (Not Before) claim in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/not_before.rb:6
class JWT::Claims::NotBefore
  # Initializes a new NotBefore instance.
  #
  # @param leeway [Integer] the amount of leeway (in seconds) to allow when validating the 'nbf' claim. Defaults to 0.
  #
  # pkg:gem/jwt#lib/jwt/claims/not_before.rb:10
  def initialize(leeway:); end

  # Verifies the 'nbf' (Not Before) claim in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::ImmatureSignature] if the 'nbf' claim has not been reached.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/not_before.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/not_before.rb:29
  def leeway; end
end

# The Numeric class is responsible for validating numeric claims in a JWT token.
# The numeric claims are: exp, iat and nbf
#
# pkg:gem/jwt#lib/jwt/claims/numeric.rb:7
class JWT::Claims::Numeric
  # Verifies the numeric claims in the JWT context.
  #
  # @param context [Object] the context containing the JWT payload.
  # @raise [JWT::InvalidClaimError] if any numeric claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/numeric.rb:42
  def verify!(context:); end

  private

  # pkg:gem/jwt#lib/jwt/claims/numeric.rb:66
  def validate_is_numeric(payload, claim); end

  # pkg:gem/jwt#lib/jwt/claims/numeric.rb:60
  def validate_numeric_claims(payload); end

  class << self
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/claims/numeric.rb:30
    def new(*args); end

    # Verifies the numeric claims in the JWT payload.
    #
    # @param payload [Hash] the JWT payload containing the claims.
    # @param _args [Hash] additional arguments (not used).
    # @raise [JWT::InvalidClaimError] if any numeric claim is invalid.
    # @return [nil]
    # @deprecated The ::JWT::Claims::Numeric.verify! method will be removed in the next major version of ruby-jwt
    #
    # pkg:gem/jwt#lib/jwt/claims/numeric.rb:53
    def verify!(payload:, **_args); end
  end
end

# The Compat class provides backward compatibility for numeric claim validation.
# @api private
#
# pkg:gem/jwt#lib/jwt/claims/numeric.rb:10
class JWT::Claims::Numeric::Compat
  # pkg:gem/jwt#lib/jwt/claims/numeric.rb:11
  def initialize(payload); end

  # pkg:gem/jwt#lib/jwt/claims/numeric.rb:15
  def verify!; end
end

# List of numeric claims that can be validated.
#
# pkg:gem/jwt#lib/jwt/claims/numeric.rb:21
JWT::Claims::Numeric::NUMERIC_CLAIMS = T.let(T.unsafe(nil), Array)

# The Required class is responsible for validating that all required claims are present in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/required.rb:6
class JWT::Claims::Required
  # Initializes a new Required instance.
  #
  # @param required_claims [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/claims/required.rb:10
  def initialize(required_claims:); end

  # Verifies that all required claims are present in the JWT payload.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::MissingRequiredClaim] if any required claim is missing.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/required.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/required.rb:30
  def required_claims; end
end

# The Subject class is responsible for validating the subject claim ('sub') in a JWT token.
#
# pkg:gem/jwt#lib/jwt/claims/subject.rb:6
class JWT::Claims::Subject
  # Initializes a new Subject instance.
  #
  # @param expected_subject [String] the expected subject for the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/claims/subject.rb:10
  def initialize(expected_subject:); end

  # Verifies the subject claim ('sub') in the JWT token.
  #
  # @param context [Object] the context containing the JWT payload.
  # @param _args [Hash] additional arguments (not used).
  # @raise [JWT::InvalidSubError] if the subject claim is invalid.
  # @return [nil]
  #
  # pkg:gem/jwt#lib/jwt/claims/subject.rb:20
  def verify!(context:, **_args); end

  private

  # pkg:gem/jwt#lib/jwt/claims/subject.rb:27
  def expected_subject; end
end

# Context class to contain the data passed to individual claim validators
#
# @api private
#
# pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
class JWT::Claims::VerificationContext < ::Struct
  # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
  def payload; end

  # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
  def payload=(_); end

  class << self
    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
    def [](*_arg0); end

    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
    def inspect; end

    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
    def keyword_init?; end

    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
    def members; end

    # pkg:gem/jwt#lib/jwt/claims/decode_verifier.rb:8
    def new(*_arg0); end
  end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/claims/verification_methods.rb:6
module JWT::Claims::VerificationMethods
  # pkg:gem/jwt#lib/jwt/claims/verification_methods.rb:11
  def claim_errors(*options); end

  # pkg:gem/jwt#lib/jwt/claims/verification_methods.rb:15
  def valid_claims?(*options); end

  # pkg:gem/jwt#lib/jwt/claims/verification_methods.rb:7
  def verify_claims!(*options); end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/claims/verifier.rb:6
module JWT::Claims::Verifier
  class << self
    # @api private
    #
    # pkg:gem/jwt#lib/jwt/claims/verifier.rb:32
    def errors(context, *options); end

    # @api private
    #
    # pkg:gem/jwt#lib/jwt/claims/verifier.rb:24
    def verify!(context, *options); end

    private

    # pkg:gem/jwt#lib/jwt/claims/verifier.rb:44
    def iterate_verifiers(*options); end

    # pkg:gem/jwt#lib/jwt/claims/verifier.rb:54
    def verify_one!(context, verifier, options); end
  end
end

# pkg:gem/jwt#lib/jwt/claims/verifier.rb:7
JWT::Claims::Verifier::VERIFIERS = T.let(T.unsafe(nil), Hash)

# @deprecated Use `Claims.verify_payload!` directly instead.
#
# pkg:gem/jwt#lib/jwt/claims_validator.rb:5
class JWT::ClaimsValidator
  # @deprecated Use `Claims.verify_payload!` directly instead.
  #
  # pkg:gem/jwt#lib/jwt/claims_validator.rb:7
  def initialize(payload); end

  # @deprecated Use `Claims.verify_payload!` directly instead.
  #
  # pkg:gem/jwt#lib/jwt/claims_validator.rb:13
  def validate!; end
end

# The Configuration module provides methods to configure JWT settings.
#
# pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:4
module JWT::Configuration
  # Returns the JWT configuration container.
  #
  # @return [JWT::Configuration::Container] the configuration container.
  #
  # pkg:gem/jwt#lib/jwt/configuration.rb:19
  def configuration; end

  # Configures the JWT settings.
  #
  # @yield [config] Gives the current configuration to the block.
  # @yieldparam config [JWT::Configuration::Container] the configuration container.
  #
  # pkg:gem/jwt#lib/jwt/configuration.rb:12
  def configure; end
end

# The Container class holds the configuration settings for JWT.
#
# pkg:gem/jwt#lib/jwt/configuration/container.rb:9
class JWT::Configuration::Container
  # Initializes a new Container instance and resets the configuration.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:23
  def initialize; end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def decode; end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def decode=(_arg0); end

  # @!attribute [r] deprecation_warnings
  #   @return [Symbol] the deprecation warnings setting.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:20
  def deprecation_warnings; end

  # Sets the deprecation warnings setting.
  #
  # @param value [Symbol] the deprecation warnings setting. Must be one of `:once`, `:warn`, or `:silent`.
  # @raise [ArgumentError] if the value is not one of the supported values.
  # @return [void]
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:45
  def deprecation_warnings=(value); end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def jwk; end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def jwk=(_arg0); end

  # Resets the configuration to default values.
  #
  # @return [void]
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:30
  def reset!; end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def strict_base64_decoding; end

  # @!attribute [rw] decode
  #   @return [DecodeConfiguration] the decode configuration.
  # @!attribute [rw] jwk
  #   @return [JwkConfiguration] the JWK configuration.
  # @!attribute [rw] strict_base64_decoding
  #   @return [Boolean] whether strict Base64 decoding is enabled.
  #
  # pkg:gem/jwt#lib/jwt/configuration/container.rb:16
  def strict_base64_decoding=(_arg0); end
end

# pkg:gem/jwt#lib/jwt/configuration/container.rb:38
JWT::Configuration::Container::DEPRECATION_WARNINGS_VALUES = T.let(T.unsafe(nil), Array)

# The DecodeConfiguration class holds the configuration settings for decoding JWT tokens.
#
# pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:6
class JWT::Configuration::DecodeConfiguration
  # Initializes a new DecodeConfiguration instance with default settings.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:40
  def initialize; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def algorithms; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def algorithms=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def leeway; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def leeway=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def required_claims; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def required_claims=(_arg0); end

  # @api private
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:54
  def to_h; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_aud; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_aud=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_expiration; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_expiration=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_iat; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_iat=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_iss; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_iss=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_jti; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_jti=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_not_before; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_not_before=(_arg0); end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_sub; end

  # @!attribute [rw] verify_expiration
  #   @return [Boolean] whether to verify the expiration claim.
  # @!attribute [rw] verify_not_before
  #   @return [Boolean] whether to verify the not before claim.
  # @!attribute [rw] verify_iss
  #   @return [Boolean] whether to verify the issuer claim.
  # @!attribute [rw] verify_iat
  #   @return [Boolean] whether to verify the issued at claim.
  # @!attribute [rw] verify_jti
  #   @return [Boolean] whether to verify the JWT ID claim.
  # @!attribute [rw] verify_aud
  #   @return [Boolean] whether to verify the audience claim.
  # @!attribute [rw] verify_sub
  #   @return [Boolean] whether to verify the subject claim.
  # @!attribute [rw] leeway
  #   @return [Integer] the leeway in seconds for time-based claims.
  # @!attribute [rw] algorithms
  #   @return [Array<String>] the list of acceptable algorithms.
  # @!attribute [rw] required_claims
  #   @return [Array<String>] the list of required claims.
  #
  # pkg:gem/jwt#lib/jwt/configuration/decode_configuration.rb:28
  def verify_sub=(_arg0); end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/configuration/jwk_configuration.rb:9
class JWT::Configuration::JwkConfiguration
  # pkg:gem/jwt#lib/jwt/configuration/jwk_configuration.rb:10
  def initialize; end

  # pkg:gem/jwt#lib/jwt/configuration/jwk_configuration.rb:25
  def kid_generator; end

  # pkg:gem/jwt#lib/jwt/configuration/jwk_configuration.rb:25
  def kid_generator=(_arg0); end

  # pkg:gem/jwt#lib/jwt/configuration/jwk_configuration.rb:14
  def kid_generator_type=(value); end
end

# The Decode class is responsible for decoding and verifying JWT tokens.
#
# pkg:gem/jwt#lib/jwt/decode.rb:8
class JWT::Decode
  # Initializes a new Decode instance.
  #
  # @param jwt [String] the JWT to decode.
  # @param key [String, Array<String>] the key(s) to use for verification.
  # @param verify [Boolean] whether to verify the token's signature.
  # @param options [Hash] additional options for decoding and verification.
  # @param keyfinder [Proc] an optional key finder block to dynamically find the key for verification.
  # @raise [JWT::DecodeError] if decoding or verification fails.
  #
  # pkg:gem/jwt#lib/jwt/decode.rb:17
  def initialize(jwt, key, verify, options, &keyfinder); end

  # Decodes the JWT token and verifies its segments if verification is enabled.
  #
  # @return [Array<Hash>] an array containing the decoded payload and header.
  #
  # pkg:gem/jwt#lib/jwt/decode.rb:30
  def decode_segments; end

  private

  # pkg:gem/jwt#lib/jwt/decode.rb:116
  def alg_in_header; end

  # pkg:gem/jwt#lib/jwt/decode.rb:87
  def allowed_algorithms; end

  # pkg:gem/jwt#lib/jwt/decode.rb:69
  def allowed_and_valid_algorithms; end

  # pkg:gem/jwt#lib/jwt/decode.rb:95
  def find_key(&keyfinder); end

  # pkg:gem/jwt#lib/jwt/decode.rb:79
  def given_algorithms; end

  # pkg:gem/jwt#lib/jwt/decode.rb:112
  def none_algorithm?; end

  # pkg:gem/jwt#lib/jwt/decode.rb:91
  def resolve_allowed_algorithms; end

  # pkg:gem/jwt#lib/jwt/decode.rb:61
  def set_key; end

  # pkg:gem/jwt#lib/jwt/decode.rb:44
  def token; end

  # pkg:gem/jwt#lib/jwt/decode.rb:103
  def validate_segment_count!; end

  # pkg:gem/jwt#lib/jwt/decode.rb:54
  def verify_algo; end

  # pkg:gem/jwt#lib/jwt/decode.rb:46
  def verify_signature; end
end

# Order is very important - first check for string keys, next for symbols
#
# pkg:gem/jwt#lib/jwt/decode.rb:74
JWT::Decode::ALGORITHM_KEYS = T.let(T.unsafe(nil), Array)

# The DecodeError class is raised when there is an error decoding a JWT.
#
# pkg:gem/jwt#lib/jwt/error.rb:8
class JWT::DecodeError < ::StandardError; end

# Deprecations module to handle deprecation warnings in the gem
# @api private
#
# pkg:gem/jwt#lib/jwt/deprecations.rb:6
module JWT::Deprecations
  class << self
    # pkg:gem/jwt#lib/jwt/deprecations.rb:8
    def context; end

    # pkg:gem/jwt#lib/jwt/deprecations.rb:32
    def emit_warnings; end

    # pkg:gem/jwt#lib/jwt/deprecations.rb:28
    def store(message); end

    # pkg:gem/jwt#lib/jwt/deprecations.rb:14
    def warning(message, only_if_valid: T.unsafe(nil)); end

    private

    # pkg:gem/jwt#lib/jwt/deprecations.rb:40
    def record_warned(message); end
  end
end

# The Encode class is responsible for encoding JWT tokens.
#
# pkg:gem/jwt#lib/jwt/encode.rb:7
class JWT::Encode
  # Initializes a new Encode instance.
  #
  # @param options [Hash] the options for encoding the JWT token.
  # @option options [Hash] :payload the payload of the JWT token.
  # @option options [Hash] :headers the headers of the JWT token.
  # @option options [String] :key the key used to sign the JWT token.
  # @option options [String] :algorithm the algorithm used to sign the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/encode.rb:15
  def initialize(options); end

  # Encodes the JWT token and returns its segments.
  #
  # @return [String] the encoded JWT token.
  #
  # pkg:gem/jwt#lib/jwt/encode.rb:24
  def segments; end
end

# The EncodeError class is raised when there is an error encoding a JWT.
#
# pkg:gem/jwt#lib/jwt/error.rb:5
class JWT::EncodeError < ::StandardError; end

# Represents an encoded JWT token
#
# Processing an encoded and signed token:
#
#   token = JWT::Token.new(payload: {pay: 'load'})
#   token.sign!(algorithm: 'HS256', key: 'secret')
#
#   encoded_token = JWT::EncodedToken.new(token.jwt)
#   encoded_token.verify_signature!(algorithm: 'HS256', key: 'secret')
#   encoded_token.payload # => {'pay' => 'load'}
#
# pkg:gem/jwt#lib/jwt/encoded_token.rb:14
class JWT::EncodedToken
  include ::JWT::Claims::VerificationMethods

  # Initializes a new EncodedToken instance.
  #
  # @param jwt [String] the encoded JWT token.
  # @raise [ArgumentError] if the provided JWT is not a String.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:25
  def initialize(jwt); end

  # Returns the encoded header of the JWT token.
  #
  # @return [String] the encoded header.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:54
  def encoded_header; end

  # Sets or returns the encoded payload of the JWT token.
  #
  # @return [String] the encoded payload.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:66
  def encoded_payload; end

  # Sets or returns the encoded payload of the JWT token.
  #
  # @return [String] the encoded payload.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:66
  def encoded_payload=(_arg0); end

  # Returns the encoded signature of the JWT token.
  #
  # @return [String] the encoded signature.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:42
  def encoded_signature; end

  # Returns the decoded header of the JWT token.
  #
  # @return [Hash] the header.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:47
  def header; end

  # Returns the original token provided to the class.
  # @return [String] The JWT token.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:19
  def jwt; end

  # Returns the payload of the JWT token.
  #
  # @return [Hash] the payload.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:59
  def payload; end

  # Returns the decoded signature of the JWT token.
  #
  # @return [String] the decoded signature.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:35
  def signature; end

  # Returns the signing input of the JWT token.
  #
  # @return [String] the signing input.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:71
  def signing_input; end

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:106
  def to_s; end

  # Checks if the signature of the JWT token is valid.
  #
  # @param algorithm [String, Array<String>, Object, Array<Object>] the algorithm(s) to use for verification.
  # @param key [String, Array<String>] the key(s) to use for verification.
  # @return [Boolean] true if the signature is valid, false otherwise.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:98
  def valid_signature?(algorithm:, key:); end

  # Verifies the signature of the JWT token.
  #
  # @param algorithm [String, Array<String>, Object, Array<Object>] the algorithm(s) to use for verification.
  # @param key [String, Array<String>] the key(s) to use for verification.
  # @param key_finder [#call] an object responding to `call` to find the key for verification.
  # @return [nil]
  # @raise [JWT::VerificationError] if the signature verification fails.
  # @raise [ArgumentError] if neither key nor key_finder is provided, or if both are provided.
  #
  # pkg:gem/jwt#lib/jwt/encoded_token.rb:83
  def verify_signature!(algorithm:, key: T.unsafe(nil), key_finder: T.unsafe(nil)); end

  private

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:110
  def decode_payload; end

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:133
  def parse(segment); end

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:125
  def parse_and_decode(segment); end

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:129
  def parse_unencoded(segment); end

  # pkg:gem/jwt#lib/jwt/encoded_token.rb:121
  def unencoded_payload?; end
end

# The ExpiredSignature class is raised when the JWT signature has expired.
#
# pkg:gem/jwt#lib/jwt/error.rb:17
class JWT::ExpiredSignature < ::JWT::DecodeError; end

# The ImmatureSignature class is raised when the JWT signature is immature.
#
# pkg:gem/jwt#lib/jwt/error.rb:23
class JWT::ImmatureSignature < ::JWT::DecodeError; end

# The IncorrectAlgorithm class is raised when the JWT algorithm is incorrect.
#
# pkg:gem/jwt#lib/jwt/error.rb:20
class JWT::IncorrectAlgorithm < ::JWT::DecodeError; end

# The InvalidAudError class is raised when the JWT audience (aud) claim is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:35
class JWT::InvalidAudError < ::JWT::DecodeError; end

# The InvalidCritError class is raised when the JWT crit header is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:41
class JWT::InvalidCritError < ::JWT::DecodeError; end

# The InvalidIatError class is raised when the JWT issued at (iat) claim is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:32
class JWT::InvalidIatError < ::JWT::DecodeError; end

# The InvalidIssuerError class is raised when the JWT issuer is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:26
class JWT::InvalidIssuerError < ::JWT::DecodeError; end

# The InvalidJtiError class is raised when the JWT ID (jti) claim is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:44
class JWT::InvalidJtiError < ::JWT::DecodeError; end

# The InvalidPayload class is raised when the JWT payload is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:47
class JWT::InvalidPayload < ::JWT::DecodeError; end

# The InvalidSubError class is raised when the JWT subject (sub) claim is invalid.
#
# pkg:gem/jwt#lib/jwt/error.rb:38
class JWT::InvalidSubError < ::JWT::DecodeError; end

# @api private
#
# pkg:gem/jwt#lib/jwt/json.rb:7
class JWT::JSON
  class << self
    # pkg:gem/jwt#lib/jwt/json.rb:9
    def generate(data); end

    # pkg:gem/jwt#lib/jwt/json.rb:13
    def parse(data); end
  end
end

# The JWA module contains all supported algorithms.
# JSON Web Algorithms
#
# pkg:gem/jwt#lib/jwt/jwa/compat.rb:4
module JWT::JWA
  class << self
    # @deprecated The `::JWT::JWA.create` method is deprecated and will be removed in the next major version of ruby-jwt.
    #
    # pkg:gem/jwt#lib/jwt/jwa.rb:52
    def create(algorithm); end

    # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:52
    def find(algo); end

    # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:48
    def register_algorithm(algo); end

    # @api private
    #
    # pkg:gem/jwt#lib/jwt/jwa.rb:34
    def resolve(algorithm); end

    # @api private
    #
    # pkg:gem/jwt#lib/jwt/jwa.rb:46
    def resolve_and_sort(algorithms:, preferred_algorithm:); end

    private

    # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:58
    def algorithms; end
  end
end

# Provides backwards compatibility for algorithms
# @api private
#
# pkg:gem/jwt#lib/jwt/jwa/compat.rb:7
module JWT::JWA::Compat
  mixes_in_class_methods ::JWT::JWA::Compat::ClassMethods

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/compat.rb:27
    def included(klass); end
  end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/jwa/compat.rb:9
module JWT::JWA::Compat::ClassMethods
  # pkg:gem/jwt#lib/jwt/jwa/compat.rb:10
  def from_algorithm(algorithm); end

  # pkg:gem/jwt#lib/jwt/jwa/compat.rb:14
  def sign(algorithm, msg, key); end

  # pkg:gem/jwt#lib/jwt/jwa/compat.rb:20
  def verify(algorithm, key, signing_input, signature); end
end

# ECDSA signing algorithm
#
# pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:6
class JWT::JWA::Ecdsa
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:9
  def initialize(alg, digest); end

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:14
  def sign(data:, signing_key:); end

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:22
  def verify(data:, signature:, verification_key:); end

  private

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:84
  def asn1_to_raw(signature, public_key); end

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:73
  def curve_by_name(name); end

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:71
  def digest; end

  # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:77
  def raw_to_asn1(signature, private_key); end

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:63
    def curve_by_name(name); end

    # pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:59
    def from_algorithm(algorithm); end
  end
end

# pkg:gem/jwt#lib/jwt/jwa/ecdsa.rb:32
JWT::JWA::Ecdsa::NAMED_CURVES = T.let(T.unsafe(nil), Hash)

# Implementation of the HMAC family of algorithms
#
# pkg:gem/jwt#lib/jwt/jwa/hmac.rb:6
class JWT::JWA::Hmac
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:13
  def initialize(alg, digest); end

  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:18
  def sign(data:, signing_key:); end

  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:24
  def verify(data:, signature:, verification_key:); end

  private

  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:36
  def digest; end

  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:38
  def ensure_valid_key!(key); end

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:9
    def from_algorithm(algorithm); end
  end
end

# Copy of https://github.com/rails/rails/blob/v7.0.3.1/activesupport/lib/active_support/security_utils.rb
#
# pkg:gem/jwt#lib/jwt/jwa/hmac.rb:45
module JWT::JWA::Hmac::SecurityUtils
  private

  # :nocov:
  #
  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:52
  def fixed_length_secure_compare(a, b); end

  # Secure string comparison for strings of variable length.
  #
  # While a timing attack would not be able to discern the content of
  # a secret compared via secure_compare, it is possible to determine
  # the secret length. This should be considered when using secure_compare
  # to compare weak, short secrets to user input.
  #
  # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:76
  def secure_compare(a, b); end

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:68
    def fixed_length_secure_compare(a, b); end

    # pkg:gem/jwt#lib/jwt/jwa/hmac.rb:79
    def secure_compare(a, b); end
  end
end

# Implementation of the none algorithm
#
# pkg:gem/jwt#lib/jwt/jwa/none.rb:6
class JWT::JWA::None
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/none.rb:9
  def initialize; end

  # pkg:gem/jwt#lib/jwt/jwa/none.rb:13
  def sign(*_arg0); end

  # pkg:gem/jwt#lib/jwt/jwa/none.rb:17
  def verify(*_arg0); end
end

# Implementation of the RSASSA-PSS family of algorithms
#
# pkg:gem/jwt#lib/jwt/jwa/ps.rb:6
class JWT::JWA::Ps
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/ps.rb:9
  def initialize(alg); end

  # pkg:gem/jwt#lib/jwt/jwa/ps.rb:14
  def sign(data:, signing_key:); end

  # pkg:gem/jwt#lib/jwt/jwa/ps.rb:20
  def verify(data:, signature:, verification_key:); end

  private

  # pkg:gem/jwt#lib/jwt/jwa/ps.rb:32
  def digest_algorithm; end
end

# Implementation of the RSA family of algorithms
#
# pkg:gem/jwt#lib/jwt/jwa/rsa.rb:6
class JWT::JWA::Rsa
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/rsa.rb:9
  def initialize(alg); end

  # pkg:gem/jwt#lib/jwt/jwa/rsa.rb:14
  def sign(data:, signing_key:); end

  # pkg:gem/jwt#lib/jwt/jwa/rsa.rb:20
  def verify(data:, signature:, verification_key:); end

  private

  # pkg:gem/jwt#lib/jwt/jwa/rsa.rb:32
  def digest; end
end

# Base functionality for signing algorithms
#
# pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:7
module JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat

  mixes_in_class_methods ::JWT::JWA::SigningAlgorithm::ClassMethods
  mixes_in_class_methods ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:20
  def alg; end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:26
  def header(*_arg0); end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:42
  def raise_sign_error!(message); end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:38
  def raise_verify_error!(message); end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:30
  def sign(*_arg0); end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:22
  def valid_alg?(alg_to_check); end

  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:34
  def verify(*_arg0); end

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:15
    def included(klass); end
  end
end

# Class methods for the SigningAlgorithm module
#
# pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:9
module JWT::JWA::SigningAlgorithm::ClassMethods
  # pkg:gem/jwt#lib/jwt/jwa/signing_algorithm.rb:10
  def register_algorithm(algo); end
end

# Represents an unsupported algorithm
#
# pkg:gem/jwt#lib/jwt/jwa/unsupported.rb:6
module JWT::JWA::Unsupported
  extend ::JWT::JWA::SigningAlgorithm
  extend ::JWT::JWA::Compat

  class << self
    # pkg:gem/jwt#lib/jwt/jwa/unsupported.rb:10
    def sign(*_arg0); end

    # pkg:gem/jwt#lib/jwt/jwa/unsupported.rb:14
    def verify(*_arg0); end
  end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:6
class JWT::JWA::Wrapper
  include ::JWT::JWA::SigningAlgorithm
  include ::JWT::JWA::Compat
  extend ::JWT::JWA::SigningAlgorithm::ClassMethods
  extend ::JWT::JWA::Compat::ClassMethods

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:9
  def initialize(algorithm); end

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:13
  def alg; end

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:25
  def header(*args, **kwargs); end

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:31
  def sign(*args, **kwargs); end

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:19
  def valid_alg?(alg_to_check); end

  # pkg:gem/jwt#lib/jwt/jwa/wrapper.rb:37
  def verify(*args, **kwargs); end
end

# JSON Web Key (JWK)
#
# pkg:gem/jwt#lib/jwt/jwk/kid_as_key_digest.rb:4
module JWT::JWK
  class << self
    # pkg:gem/jwt#lib/jwt/jwk.rb:25
    def classes; end

    # pkg:gem/jwt#lib/jwt/jwk.rb:10
    def create_from(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

    # pkg:gem/jwt#lib/jwt/jwk.rb:31
    def import(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

    # pkg:gem/jwt#lib/jwt/jwk.rb:30
    def new(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

    private

    # pkg:gem/jwt#lib/jwt/jwk.rb:39
    def generate_mappings; end

    # pkg:gem/jwt#lib/jwt/jwk.rb:35
    def mappings; end
  end
end

# JWK representation for Elliptic Curve (EC) keys
#
# pkg:gem/jwt#lib/jwt/jwk/ec.rb:8
class JWT::JWK::EC < ::JWT::JWK::KeyBase
  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:17
  def initialize(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:68
  def []=(key, value); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:55
  def export(options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:61
  def key_digest; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:31
  def keypair; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:51
  def members; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:35
  def private?; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:47
  def public_key; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:39
  def signing_key; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:43
  def verify_key; end

  private

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:94
  def check_jwk_params!(key_params, params); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:144
  def create_ec_key(jwk_crv, jwk_x, jwk_y, jwk_d); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:207
  def decode_octets(base64_encoded_coordinate); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:76
  def ec_key; end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:121
  def encode_octets(octets); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:127
  def encode_open_ssl_bn(key_part); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:80
  def extract_key_params(key); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:100
  def keypair_components(ec_keypair); end

  # pkg:gem/jwt#lib/jwt/jwk/ec.rb:131
  def parse_ec_key(key); end

  class << self
    # pkg:gem/jwt#lib/jwt/jwk/ec.rb:231
    def import(jwk_data); end

    # pkg:gem/jwt#lib/jwt/jwk/ec.rb:235
    def to_openssl_curve(crv); end
  end
end

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:11
JWT::JWK::EC::BINARY = T.let(T.unsafe(nil), Integer)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:14
JWT::JWK::EC::EC_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:13
JWT::JWK::EC::EC_PRIVATE_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:12
JWT::JWK::EC::EC_PUBLIC_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:9
JWT::JWK::EC::KTY = T.let(T.unsafe(nil), String)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:10
JWT::JWK::EC::KTYS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/ec.rb:15
JWT::JWK::EC::ZERO_BYTE = T.let(T.unsafe(nil), String)

# JWK for HMAC keys
#
# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:6
class JWT::JWK::HMAC < ::JWT::JWK::KeyBase
  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:13
  def initialize(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:64
  def []=(key, value); end

  # See https://tools.ietf.org/html/rfc7517#appendix-A.3
  #
  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:48
  def export(options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:58
  def key_digest; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:27
  def keypair; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:54
  def members; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:31
  def private?; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:35
  def public_key; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:43
  def signing_key; end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:39
  def verify_key; end

  private

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:89
  def check_jwk(keypair, params); end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:76
  def extract_key_params(key); end

  # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:72
  def secret; end

  class << self
    # pkg:gem/jwt#lib/jwt/jwk/hmac.rb:96
    def import(jwk_data); end
  end
end

# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:11
JWT::JWK::HMAC::HMAC_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:10
JWT::JWK::HMAC::HMAC_PRIVATE_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:9
JWT::JWK::HMAC::HMAC_PUBLIC_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:7
JWT::JWK::HMAC::KTY = T.let(T.unsafe(nil), String)

# pkg:gem/jwt#lib/jwt/jwk/hmac.rb:8
JWT::JWK::HMAC::KTYS = T.let(T.unsafe(nil), Array)

# Base for JWK implementations
#
# pkg:gem/jwt#lib/jwt/jwk/key_base.rb:6
class JWT::JWK::KeyBase
  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:12
  def initialize(options, params = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:47
  def <=>(other); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:41
  def ==(other); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:33
  def [](key); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:37
  def []=(key, value); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:45
  def eql?(other); end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:29
  def hash; end

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:25
  def kid; end

  private

  # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:55
  def parameters; end

  class << self
    # pkg:gem/jwt#lib/jwt/jwk/key_base.rb:7
    def inherited(klass); end
  end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/jwk/key_finder.rb:6
class JWT::JWK::KeyFinder
  # pkg:gem/jwt#lib/jwt/jwk/key_finder.rb:7
  def initialize(options); end

  # pkg:gem/jwt#lib/jwt/jwk/key_finder.rb:18
  def key_for(kid); end

  private

  # pkg:gem/jwt#lib/jwt/jwk/key_finder.rb:32
  def resolve_key(kid); end
end

# @api private
#
# pkg:gem/jwt#lib/jwt/jwk/kid_as_key_digest.rb:6
class JWT::JWK::KidAsKeyDigest
  # pkg:gem/jwt#lib/jwt/jwk/kid_as_key_digest.rb:7
  def initialize(jwk); end

  # pkg:gem/jwt#lib/jwt/jwk/kid_as_key_digest.rb:11
  def generate; end
end

# JSON Web Key (JWK) representation of a RSA key
#
# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:6
class JWT::JWK::RSA < ::JWT::JWK::KeyBase
  # https://www.rfc-editor.org/rfc/rfc3447#appendix-A.1.2
  #
  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:17
  def initialize(key, params = T.unsafe(nil), options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:67
  def []=(key, value); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:51
  def export(options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:61
  def key_digest; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:31
  def keypair; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:57
  def members; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:35
  def private?; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:39
  def public_key; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:43
  def signing_key; end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:47
  def verify_key; end

  private

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:93
  def check_jwk_params!(key_params, params); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:125
  def decode_open_ssl_bn(jwk_data); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:119
  def encode_open_ssl_bn(key_part); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:79
  def extract_key_params(key); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:113
  def jwk_attributes(*attributes); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:99
  def parse_rsa_key(key); end

  # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:75
  def rsa_key; end

  class << self
    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:193
    def create_rsa_key(rsa_parameters); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:168
    def create_rsa_key_using_accessors(rsa_parameters); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:140
    def create_rsa_key_using_der(rsa_parameters); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:158
    def create_rsa_key_using_sets(rsa_parameters); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:134
    def decode_open_ssl_bn(jwk_data); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:130
    def import(jwk_data); end

    # pkg:gem/jwt#lib/jwt/jwk/rsa.rb:183
    def validate_rsa_parameters!(rsa_parameters); end
  end
end

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:7
JWT::JWK::RSA::BINARY = T.let(T.unsafe(nil), Integer)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:8
JWT::JWK::RSA::KTY = T.let(T.unsafe(nil), String)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:9
JWT::JWK::RSA::KTYS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:15
JWT::JWK::RSA::RSA_ASN1_SEQUENCE = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:12
JWT::JWK::RSA::RSA_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:14
JWT::JWK::RSA::RSA_OPT_PARAMS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:11
JWT::JWK::RSA::RSA_PRIVATE_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# pkg:gem/jwt#lib/jwt/jwk/rsa.rb:10
JWT::JWK::RSA::RSA_PUBLIC_KEY_ELEMENTS = T.let(T.unsafe(nil), Array)

# JSON Web Key Set (JWKS) representation
# https://tools.ietf.org/html/rfc7517
#
# pkg:gem/jwt#lib/jwt/jwk/set.rb:9
class JWT::JWK::Set
  include ::Enumerable
  extend ::Forwardable

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:15
  def initialize(jwks = T.unsafe(nil), options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:78
  def +(enum); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:79
  def <<(key); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:69
  def ==(other); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:64
  def add(key); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:37
  def delete(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:37
  def dig(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:37
  def each(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:73
  def eql?(other); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:33
  def export(options = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:74
  def filter!(&block); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:13
  def keys; end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:75
  def length(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:55
  def merge(enum); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:45
  def reject!(&block); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:39
  def select!(&block); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:37
  def size(*_arg0, **_arg1, &_arg2); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:60
  def union(enum); end

  # pkg:gem/jwt#lib/jwt/jwk/set.rb:51
  def uniq!(&block); end

  # For symbolic manipulation
  #
  # pkg:gem/jwt#lib/jwt/jwk/set.rb:77
  def |(enum); end
end

# https://tools.ietf.org/html/rfc7638
#
# pkg:gem/jwt#lib/jwt/jwk/thumbprint.rb:6
class JWT::JWK::Thumbprint
  # pkg:gem/jwt#lib/jwt/jwk/thumbprint.rb:9
  def initialize(jwk); end

  # pkg:gem/jwt#lib/jwt/jwk/thumbprint.rb:13
  def generate; end

  # pkg:gem/jwt#lib/jwt/jwk/thumbprint.rb:7
  def jwk; end

  # pkg:gem/jwt#lib/jwt/jwk/thumbprint.rb:23
  def to_s; end
end

# The JWKError class is raised when there is an error with the JSON Web Key (JWK).
#
# pkg:gem/jwt#lib/jwt/error.rb:56
class JWT::JWKError < ::JWT::DecodeError; end

# The MissingRequiredClaim class is raised when a required claim is missing from the JWT.
#
# pkg:gem/jwt#lib/jwt/error.rb:50
class JWT::MissingRequiredClaim < ::JWT::DecodeError; end

# The RequiredDependencyError class is raised when a required dependency is missing.
#
# pkg:gem/jwt#lib/jwt/error.rb:11
class JWT::RequiredDependencyError < ::StandardError; end

# Represents a JWT token
#
# Basic token signed using the HS256 algorithm:
#
#   token = JWT::Token.new(payload: {pay: 'load'})
#   token.sign!(algorithm: 'HS256', key: 'secret')
#   token.jwt # => eyJhb....
#
# Custom headers will be combined with generated headers:
#   token = JWT::Token.new(payload: {pay: 'load'}, header: {custom: "value"})
#   token.sign!(algorithm: 'HS256', key: 'secret')
#   token.header # => {"custom"=>"value", "alg"=>"HS256"}
#
# pkg:gem/jwt#lib/jwt/token.rb:17
class JWT::Token
  include ::JWT::Claims::VerificationMethods

  # Initializes a new Token instance.
  #
  # @param header [Hash] the header of the JWT token.
  # @param payload [Hash] the payload of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:24
  def initialize(payload:, header: T.unsafe(nil)); end

  # Detaches the payload according to https://datatracker.ietf.org/doc/html/rfc7515#appendix-F
  #
  # pkg:gem/jwt#lib/jwt/token.rb:84
  def detach_payload!; end

  # Returns the encoded header of the JWT token.
  #
  # @return [String] the encoded header of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:51
  def encoded_header; end

  # Returns the encoded payload of the JWT token.
  #
  # @return [String] the encoded payload of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:63
  def encoded_payload; end

  # Returns the encoded signature of the JWT token.
  #
  # @return [String] the encoded signature of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:39
  def encoded_signature; end

  # Returns the decoded header of the JWT token.
  #
  # @return [Hash] the header of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:46
  def header; end

  # Returns the JWT token as a string.
  #
  # @return [String] the JWT token as a string.
  # @raise [JWT::EncodeError] if the token is not signed or other encoding issues
  #
  # pkg:gem/jwt#lib/jwt/token.rb:78
  def jwt; end

  # Returns the payload of the JWT token.
  #
  # @return [Hash] the payload of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:58
  def payload; end

  # Signs the JWT token.
  #
  # @param algorithm [String, Object] the algorithm to use for signing.
  # @param key [String] the key to use for signing.
  # @return [void]
  # @raise [JWT::EncodeError] if the token is already signed or other problems when signing
  #
  # pkg:gem/jwt#lib/jwt/token.rb:96
  def sign!(algorithm:, key:); end

  # Returns the decoded signature of the JWT token.
  #
  # @return [String] the decoded signature of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:32
  def signature; end

  # Returns the signing input of the JWT token.
  #
  # @return [String] the signing input of the JWT token.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:70
  def signing_input; end

  # Returns the JWT token as a string.
  #
  # @return [String] the JWT token as a string.
  #
  # pkg:gem/jwt#lib/jwt/token.rb:110
  def to_s; end
end

# The UnsupportedEcdsaCurve class is raised when the ECDSA curve is unsupported.
#
# pkg:gem/jwt#lib/jwt/error.rb:29
class JWT::UnsupportedEcdsaCurve < ::JWT::IncorrectAlgorithm; end

# @api private
#
# pkg:gem/jwt#lib/jwt/version.rb:16
module JWT::VERSION; end

# pkg:gem/jwt#lib/jwt/version.rb:17
JWT::VERSION::MAJOR = T.let(T.unsafe(nil), Integer)

# pkg:gem/jwt#lib/jwt/version.rb:18
JWT::VERSION::MINOR = T.let(T.unsafe(nil), Integer)

# pkg:gem/jwt#lib/jwt/version.rb:20
JWT::VERSION::PRE = T.let(T.unsafe(nil), T.untyped)

# pkg:gem/jwt#lib/jwt/version.rb:22
JWT::VERSION::STRING = T.let(T.unsafe(nil), String)

# pkg:gem/jwt#lib/jwt/version.rb:19
JWT::VERSION::TINY = T.let(T.unsafe(nil), Integer)

# The VerificationError class is raised when there is an error verifying a JWT.
#
# pkg:gem/jwt#lib/jwt/error.rb:14
class JWT::VerificationError < ::JWT::DecodeError; end

# @deprecated This class is deprecated and will be removed in the next major version of ruby-jwt.
#
# pkg:gem/jwt#lib/jwt/verify.rb:7
class JWT::Verify
  # @deprecated This class is deprecated and will be removed in the next major version of ruby-jwt.
  #
  # pkg:gem/jwt#lib/jwt/verify.rb:28
  def initialize(payload, options); end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_aud; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_expiration; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_iat; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_iss; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_jti; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_not_before; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_required_claims; end

  # pkg:gem/jwt#lib/jwt/verify.rb:35
  def verify_sub; end

  class << self
    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_aud(payload, options); end

    # @deprecated This method is deprecated and will be removed in the next major version of ruby-jwt.
    #
    # pkg:gem/jwt#lib/jwt/verify.rb:20
    def verify_claims(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_expiration(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_iat(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_iss(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_jti(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_not_before(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_required_claims(payload, options); end

    # pkg:gem/jwt#lib/jwt/verify.rb:14
    def verify_sub(payload, options); end
  end
end

# pkg:gem/jwt#lib/jwt/verify.rb:8
JWT::Verify::DEFAULTS = T.let(T.unsafe(nil), Hash)

# pkg:gem/jwt#lib/jwt/verify.rb:9
JWT::Verify::METHODS = T.let(T.unsafe(nil), Array)

# If the x5c header certificate chain can be validated by trusted root
# certificates, and none of the certificates are revoked, returns the public
# key from the first certificate.
# See https://tools.ietf.org/html/rfc7515#section-4.1.6
#
# pkg:gem/jwt#lib/jwt/x5c_key_finder.rb:8
class JWT::X5cKeyFinder
  # pkg:gem/jwt#lib/jwt/x5c_key_finder.rb:9
  def initialize(root_certificates, crls = T.unsafe(nil)); end

  # pkg:gem/jwt#lib/jwt/x5c_key_finder.rb:15
  def from(x5c_header_or_certificates); end

  private

  # pkg:gem/jwt#lib/jwt/x5c_key_finder.rb:33
  def build_store(root_certificates, crls); end

  # pkg:gem/jwt#lib/jwt/x5c_key_finder.rb:42
  def parse_certificates(x5c_header_or_certificates); end
end
